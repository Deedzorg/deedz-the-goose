export function drawWorld(ctx, game) {
  const { cam, level, player, cat, crumbs, honks } = game;
  ...
  drawGoose(ctx, player, cam, game.time);
  drawHud(ctx, game);
}
...
function drawCat(ctx, cat, cam, t) { const x=cat.x-cam,y=cat.y; ctx.fillStyle='#020617'; ctx.beginPath(); ctx.ellipse(x,y-22,30,17,0,0,Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(x+24,y-31,16,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#86efac'; ctx.fillRect(x+19,y-34,5,5); ctx.fillRect(x+31,y-34,5,5); }
function drawGoose(ctx, p, cam, t) { const x=p.x-cam, y=p.y; if(p.inv && Math.floor(t*20)%2===0)return; ctx.fillStyle='rgba(0,0,0,.25)'; ctx.beginPath(); ctx.ellipse(x,y+4,42,10,0,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#f8fafc'; ctx.beginPath(); ctx.ellipse(x,y-52,38,48,0,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#dc2626'; roundRect(ctx,x-33,y-82,66,56,16,true); ctx.fillStyle='#f8fafc'; ctx.beginPath(); ctx.arc(x+18*p.facing,y-108,32,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#f59e0b'; ctx.beginPath(); ctx.moveTo(x+40*p.facing,y-108); ctx.lineTo(x+82*p.facing,y-98); ctx.lineTo(x+40*p.facing,y-86); ctx.fill(); ctx.fillStyle='black'; ctx.beginPath(); ctx.arc(x+26*p.facing,y-116,4,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#facc15'; ctx.fillRect(x-20,y-72,40,8); ctx.fillStyle='#111827'; ctx.fillRect(x-27,y-18,22,10); ctx.fillRect(x+8,y-18,22,10); }
function drawHud(ctx, g) { ctx.fillStyle='rgba(15,23,42,.72)'; roundRect(ctx,18,18,310,96,14,true); ctx.fillStyle='white'; ctx.font='22px system-ui'; ctx.fillText(`HP: ${g.player.hp}   Score: ${g.player.score}`,34,54); ctx.fillText(`Ammo: ${g.player.ammo}   Jumps: ${g.player.maxJumps}`,34,86); if(g.mode!=='play'){ ctx.fillStyle='rgba(15,23,42,.65)'; ctx.fillRect(0,0,1280,720); ctx.fillStyle='#facc15'; ctx.font='76px system-ui'; ctx.fillText('DEEDZ THE GOOSE',315,300); ctx.fillStyle='white'; ctx.font='28px system-ui'; ctx.fillText('Press Enter or Start to play',480,355); }}
