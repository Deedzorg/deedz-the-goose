...
export class GooseInteractionSystem {
  ...
  #interact(player) {
    ...
    // Keep interaction available without interrupting solo play with a failure toast.
    if (!remote) return;
    ...
    this.engine.network.sendAction('wing-bump', {
      targetId: remote.peerId,
      x: player.x,
      y: player.y,
  ...
  #remoteAction(payload = {}) {
    ...
    if (payload.action === 'honk') {
      ...
      const projectile = new CrumbProjectile({
        ...
        ownerId: payload.id,
        remote: true,
        strength: Number(payload.strength) || 790,
        lift: Number(payload.lift) || 155,
        duration: Number(payload.duration) || 2.2,
        damage: Number(payload.damage) || 1,
        charge,
        color: remote.character?.accent ?? 0xf7ca76,
      });
